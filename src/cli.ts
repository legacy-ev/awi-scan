...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (arg === "--format") options.format = takeValue(args, ++index, "--format");
    else if (arg.startsWith("--format=")) options.format = arg.slice("--format=".length);
    else if (arg === "--output" || arg === "-o") options.output = takeValue(args, ++index, "--output");
    else if (arg.startsWith("--output=")) options.output = arg.slice("--output=".length);
    else if (arg === "--fail-on") options.failOn = takeValue(args, ++index, "--fail-on");
